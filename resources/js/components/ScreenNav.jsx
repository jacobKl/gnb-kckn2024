...
function ScreenNav({ setScreen, screen, setCMActive }) {
  return (
    <div className="bg-white flex justify-center items-center gap-10 h-[70px] dark:bg-gray-800 text-gray-400">
      ...
      <div className={`flex flex-col cursor-pointer max-w-[120px] ${screen == 1 ? 'text-gray-600' : ''}`} onClick={() => setScreen(1)}>
        <FontAwesomeIcon icon={faList} />
        <span className="text-xs mt-1">Wszystkie linie</span>
      </div>
      <div className={`flex flex-col cursor-pointer max-w-[120px] ${screen == 2 ? 'text-gray-600' : ''}`} onClick={() => setScreen(2)}>
